<template>
  <div v-if="data" class="relative mb-12">
    <div class="whitespace-nowrap transition-all -mb-2"
      :style="{ transform: `translateX(-${state.translateX > 0 ? state.translateX : 0}%)` }">
      <a v-for="(project, index) in data" :key="project.id" href="/"
    ...
    <ClientOnly>
      <button v-if="state.activeIndex > 0" aria-label="previous image" type="button" className="button prev"
        @click="handlePrev">
        <i class="uil uil-angle-left-b" />
      </button>
      <button v-if="state.activeIndex < data.length - 1" aria-label="next image" type="button" className="button next"
        @click="handleNext">
